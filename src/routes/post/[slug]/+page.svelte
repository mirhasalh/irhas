<script lang="ts">
  ...
  const keywords = $derived([data.post.title, ...data.post.tags]),
    f = $derived(`https://www.facebook.com/sharer/sharer.php?u=${website}/post/${data.post.slug}`),
    x = $derived(`https://twitter.com/intent/tweet?text=Check%20this%20out!&url=${website}/post/${data.post.slug}`),
    l = $derived(`https://www.linkedin.com/sharing/share-offsite/?url=${website}/post/${data.post.slug}`),
    w = $derived(`https://wa.me/?text=Check%20this%20out!%20${website}/post/${data.post.slug}`),
    t = $derived(`https://t.me/share/url?url=${website}/post/${data.post.slug}&text=Check%20this%20out!`),
